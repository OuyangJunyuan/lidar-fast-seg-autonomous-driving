...
public:
    ...
    PatchWork() {
        ...
        cout << (boost::format("Num. sectors: %d, %d, %d, %d")
                 ...
                 % num_sectors_each_zone_[2] % num_sectors_each_zone_[3]).str() << endl;
        cout << (boost::format("Num. rings: %01d, %01d, %01d, %01d")
                 % num_rings_each_zone_[0] % num_rings_each_zone_[1]
                 % num_rings_each_zone_[2] % num_rings_each_zone_[3]).str() << endl;
        ...
        sector_sizes = {2 * M_PI / num_sectors_each_zone_.at(0),
...
private:
    ...
    double uprightness_thr_ = 0.707; // 0.707 for 45° and 0.866 for 60°
...
inline void PatchWork<PointT>::estimate_ground(
    ...
    sort(laserCloudIn.points.begin(), laserCloudIn.end(), [](const auto &pa, const auto &pb) { return pa.z < pb.z; });
    ...
    for (int k = 0; k < num_zones_; ++k) {
        ...
        for (uint16_t ring_idx = 0; ring_idx < num_rings_each_zone_[k]; ++ring_idx) { //{2, 4, 4, 4};
            for (uint16_t sector_idx = 0; sector_idx < num_sectors_each_zone_[k]; ++sector_idx) { //{16, 32, 54, 32};
                // 遍历每个扇区
